...
contract ObligationManager {
    ...
    event ObligationVerified(string cid, Status status);
    ...
    function createObligation(
        ...
        require(obligations[cid].startTime == 0, "Already exists");
    ...
    function updateStatus(string memory cid, Status newStatus) external onlyArbiter {
        require(obligations[cid].startTime != 0, "Obligation doesn't exist");
        obligations[cid].status = newStatus;
        emit ObligationVerified(cid, newStatus);
    }
}
